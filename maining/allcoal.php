...
if(isset($_SESSION['cook'])){
        ...
        for($num=0;$num<$nums;$num++){
            if(!empty($data[giveaways][$num][code]) and $coal >= $data[giveaways][$num][price]){
                ...
                if(preg_match('/status/i', $enter)){
                    ...
                    $buys[] = 'Ошибка:'.$data[giveaways][$num][game][name].'<br>';
                }
    ...
    //print_r($buys);
    echo 'Если вам помог сайт и вы хотите отблагодарить автора,R411205745723 :D<br>';
...
function getPage($num){
    global $cook;
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, 'http://gameminer.ru/api/giveaways/coal?page='.$num.'&count=8&q=&type=any&sortby=finish&order=asc');
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($curl, CURLOPT_USERAGENT, $agent);
    curl_setopt($curl, CURLOPT_FOLLOWLOCATION, true);
    curl_setopt($curl, CURLOPT_COOKIEFILE, $_SERVER['DOCUMENT_ROOT']."/maining/users/$cook.txt");
